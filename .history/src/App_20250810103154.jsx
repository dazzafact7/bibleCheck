...
const BibleFactChecker: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'input' | 'tree' | 'details'>('input');

  // Cache handling
  const cacheResult = (key: string, data: AnalysisResult) => {
  ...
  const analyzeTheses = async (thesisA: string, thesisB: string): Promise<AnalysisResult> => {
    // Simulate API delay
  ...
  const buildDecisionTree = useCallback((result: AnalysisResult) => {
    const newNodes: Node[] = [];
    const newEdges: Edge[] = [];
    ...
    result.claimsA.forEach((claim, index) => {
      ...
      newNodes.push({
        ...
        data: {
          label: (
            <div className="bg-blue-100 text-blue-900 p-2 rounded-lg shadow max-w-xs cursor-pointer" onClick={() => {
              const pair = result.pairs.find(p => p.a === claim.text);
              if (pair) setSelectedPair(pair);
              setActiveTab('details');
            }}>
              <div className="text-xs font-semibold">Claim A{index + 1}</div>
              <div className="text-xs mt-1">{claim.text.length > 40 ? claim.text.substring(0, 40) + '...' : claim.text}</div>
            </div>
          )
        },
        type: 'default'
    ...
    result.claimsB.forEach((claim, index) => {
      ...
      newNodes.push({
        ...
        data: {
          label: (
            <div className="bg-green-100 text-green-900 p-2 rounded-lg shadow max-w-xs cursor-pointer" onClick={() => {
      ...
      data: {
        label: (
          <div className={clsx(
            ...
            <div className="text-xs opacity-90">Contradiction Rate: {(result.result.contradiction_rate * 100).toFixed(0)}%</div>
