...
const getCacheKey=(key,num=8)=>md5(key).slice(0,num);

// Lädt Umgebungsvariablen aus der .env-Datei im Projekt-Root
dotenv.config({path: "../.env"});
const models=["moonshotai/kimi-k2:free"];
...
export default async function askGpt(systemPrompt, userPrompt,cacheKey=false,model=[]) {
const cacheKey=getCacheKey(systemPrompt+userPrompt);
const cacheFile=path.join(cacheDir,cacheKey+".json");
try{
  if(cacheKey){
    const cacheData=readFileSync(cacheFile,"utf-8");
    const cacheJson=jsonParse(cacheData);
    if(cacheJson.result){
