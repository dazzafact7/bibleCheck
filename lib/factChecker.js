...
// 0. These und Antithese aus User-Frage generieren
const generateThesesFromQuestion = async (question, model = 'moonshotai/kimi-k2') => {
  const systemPrompt = `Du bist ein Experte für theologische Dialektik. 
    Erstelle aus einer Frage zwei gegensätzliche biblische Thesen (These und Antithese).
    Zusätzlich erstelle eine kompakte 2-4 Wörter Phrase für DB-Zuordnung.
    Antworte als JSON: {
      thesisA: string,
      thesisB: string, 
      topicPhrase: string,
  ...
  const userPrompt = `Frage: "${question}"
    ...
    Erstelle:
    1. Eine positive These (thesisA)
    2. Eine gegensätzliche Antithese (thesisB)  
    3. Eine kompakte Phrase (2-4 Wörter) für die DB-Zuordnung
    
    Beispiel:
    Frage: "Ist der Sabbat biblisch?"
    thesisA: "Der Sabbat ist für Christen weiterhin gültig"
    thesisB: "Der Sabbat ist für Christen nicht mehr bindend"  
    topicPhrase: "Sabbatgültigkeit für Christen"`;
  
  const result = await ask(model, systemPrompt, userPrompt);
  return {
    thesisA: result.thesisA || '',
    thesisB: result.thesisB || '',
    topicPhrase: result.topicPhrase || '',
    originalQuestion: question
  };
};

// 1. Claims extrahieren mit biblischem Kontext
const extractClaims = async (thesis, model = 'moonshotai/kimi-k2') => {
  const systemPrompt = `Du bist ein Experte für biblische Textanalyse. 
    Zerlege theologische Aussagen in präzise, überprüfbare verständliche Einzelaussagen (Claims).
    Berücksichtige biblischen Kontext und theologische Implikationen. Ohne schwer verständlich zu argumentieren!
...
const nliJudge = async (premiseText, hypothesisText, model = 'moonshotai/kimi-k2') => {
  const systemPrompt = `Du bist ein theologischer NLI-Judge.
    ...
    Gib zusätzlich einen Score (float, 2 decimals) von 0.00 bis 1.00 an: 0.50 ist neutral, 1.00 für starke Unterstützung, 0.00 für starken Widerspruch. Verwende einfache, verständliche Argumentation.
...
const debate = async (claimA, claimB, model = 'moonshotai/kimi-k2') => {
  const systemPrompt = `Du bist ein theologischer Debattenexperte.
    Erstelle zwei kurze, aber überzeugende Plädoyers.
    Nutze biblische Argumente und Logik. Verwende einfache, verständliche Argumentation.
...
const finalJudge = async (data, model = 'moonshotai/kimi-k2') => {
  const systemPrompt = `Du bist der finale theologische Richter.
    Bewerte die Stringenz, biblische Kohärenz und logische Konsistenz der Argumente. Verwende einfache, verständliche Sprache.
...
// Neue Hauptfunktion: Analyse aus einer User-Frage
export async function analyzeQuestionDialectically(question, useDebate = true) {
  try {
    // 1. These und Antithese generieren
    const thesesData = await generateThesesFromQuestion(question);
    
    // 2. Normale Analyse mit generierten Thesen
    const analysisResult = await compareTheses(thesesData.thesisA, thesesData.thesisB, useDebate);
    
    // 3. Ergebnis mit ursprünglicher Frage und Metadaten erweitern
    return {
      ...analysisResult,
      originalQuestion: question,
      topicPhrase: thesesData.topicPhrase,
      generatedTheses: {
        thesisA: thesesData.thesisA,
        thesisB: thesesData.thesisB
      }
    };
  } catch (error) {
...
// Ursprüngliche Funktion für direkte Thesenvergleiche (Rückwärtskompatibilität)
